...
def load_raw_data_for_correlation(
    ...
    feature_cols = [c for c in df.columns if c not in set(drop_metadata_cols) | set(y_cols)]
    ordered_cols = feature_cols + list(y_cols)
    ...
    if output_len is not None and len(y_cols) != int(output_len):
        print(
            f"[WARN] Raw-correlation target count={len(y_cols)} "
