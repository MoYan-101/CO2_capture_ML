...
def visualize_main(config_path=None):
    ...
    if os.path.exists(raw_csv_path):
        ...
        if config["evaluation"].get("save_data_analysis_plots", False):
            ...
            log_eps = float(dl_cfg.get("log_transform_eps", 1e-8))
            log_cols = list(dl_cfg.get("log_transform_cols", []) or [])
            for col in log_cols:
                ln_col = f"{col} (LN scale)"
                if ln_col in df_plot.columns or col not in df_plot.columns:
                    continue
                raw = pd.to_numeric(df_plot[col], errors="coerce").clip(lower=log_eps)
                df_plot[ln_col] = np.log(raw)

            possible_cols = config["evaluation"].get("data_analysis_cols")
